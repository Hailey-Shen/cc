...
function preload() {
  song = loadSound("data/quack.mp3");
  image = loadImage("data/quack.jpeg");
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  textAlign(CENTER, CENTER);
  fontsize = 80; // the 'floor' for my text
...
function draw() {
  ...
  if (song.isPlaying()) {
    background(150,190,220,4);
  volume = analyzer.getLevel();  //<~~~only for viz
  let fontsize = map(volume,0,1,180,280);
  fill(230,230,150);
  textSize(fontsize);
  text("Quack",width/2, height/2);
  }
  else {textSize(124);}
}
...
function mousePressed() {
  ...
  if (song.isPlaying()) {
    ...
    fill(255);
    textSize(24);
    text('Playing - click to pause', width / 2, height / 3);
  }
}
