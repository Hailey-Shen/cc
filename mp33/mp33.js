...
function draw() {
  ...
  if (song.isPlaying()) {
    background(240,240,180,4)
  volume = analyzer.getLevel();  //<~~~only for viz
  let circleSize = map(volume,0,1,height,width);
  noStroke();
  fill(160,210,230);
  circle(circleSize,random(width),random(height));
  }
}
...
function mousePressed() {
  ...
  if (song.isPlaying()) {
    ...
    fill(255);
    textSize(24);
    text('Playing - click to pause', width / 4, height / 2);
  }
}
