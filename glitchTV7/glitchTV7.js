...
function preload() {
    ...
    img = loadImage('data/IMG_3855.JPG');
    img2 = loadImage('data/IMG_1778.jpeg');
}
...
function draw() {
    ...
    backgroundOpacity += backgroundFadeDirection * 5; // Adjust fade speed here (0.01 = 1% per frame)
    ...
    else { image(img, random(width), random(height), random(width), random(height), 0, 0, img.width, img.height, COVER);
    }
    
    if (random() < 0.01) {
        img2Xoffset = random(-150, 150); // big horizontal glitchy shift
        image(img2, img2Xoffset, 0, width, height, 0, 0, img2.width, img2.height, COVER);
    }
    else { image(img2, random(width), random(height), random(width), random(height), 0, 0, img2.width, img2.height, COVER);
    }
    ...
    let glitchSat = random(0, 50);    // Random saturation
    ...
    for (let i = 0; i < 15; i++) {
        ...
        let alpha = random(0, 5);  // Very transparent
